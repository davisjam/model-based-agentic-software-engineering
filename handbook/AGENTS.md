...
The handbook has **no MAGE-Moment insets**. *Model-Based Agentic Software Engineering* may appear where it is intellectually relevant — the Preface, a citation or case study, the final Coda — but it does not get a recurring branded pedagogical device, and the handbook owns its claims as software engineering rather than periodically interpreting itself through MAGE. "MAGE Moment" is a classroom convention for the lecture/course materials, where it usefully makes the course-to-book connection visible; it is not part of the handbook's literary language. Prose that would have filled such an inset either stands as ordinary body text or is deleted where the surrounding argument already lands it.
