...
*The operating stance of the `self-operations` skill: how to run an agent-fleet substrate as a DevOps engineer. The skill's `SKILL.md` carries the lifecycle map and the bootstrap; these principles are the mindset you operate with. Partner to the `self-governance` skill.*
