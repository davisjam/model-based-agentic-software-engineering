...
def heading_rows(include_appendices: bool = False) -> list[HeadingRow]:
    ...
    close marker is skipped whole (via `book_ir.blocks_outside_worked_examples`, the shared span helper at the
    marker SSOT), so the gallery's example-heads never become sections (and so never draw a spurious O2 'no
    topic sentence' / O3 / O4 finding). Topic-sentence + points still index into the FULL block list, so their
    positional lookups are unaffected by the skip."""
    doc = book_ir.parse_book(include_appendices=include_appendices)
    rows: list[HeadingRow] = []
    for c in doc.chapters:
        for b in book_ir.blocks_outside_worked_examples(c.blocks):
