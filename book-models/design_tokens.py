...
emits each renderer's native form so the three surfaces (site / web book / PDF) cannot diverge:
    ...
    css_book_dark_block() -> the book token vars re-declared from palette-dark under the slate scheme
                            (book_mkdocs.py appends this to the derived mage-book.css — dark reading plane)
...
class Tokens:
    ...
    def palette_dark(self) -> dict:
        ...
        (family shell) and css_book_dark_block() (book content plane). Kept OUT of `palette` so
        svg_palette() and css_root_block() never see the dark hexes."""
...
def css_book_dark_block(t: Tokens | None = None) -> str:
    """The BOOK content plane's dark-scheme remap: every `palette-dark` key that shadows a light
    `palette` key re-declares its `--<key>` custom property under Material's slate scheme. The
    derived book stylesheet (mage-book.css) colors ONLY through var(--…), so appending this block
    re-skins the whole reading plane — body ink, asides, apparatus frames, semantic boxes, appendix
    cards — for dark mode with zero hand-authored hex. Consumed by book_mkdocs.py's `_derive_css`;
    family-shell-only keys (`header`) are skipped automatically (absent from `palette`). The
    selector sits on <body>, below :root in the cascade tree, so the dark values win by
    inheritance, not specificity."""
    t = t or load()
    lines: list[str] = [
    ...
    for key, val in t.palette_dark.items():
        if key in t.palette:
            lines.append(f"  --{key}: {val};")
    lines.append("}")
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("cmd", choices=["css", "css-material", "css-book-dark", "typst", "palette",
                                    "figure-styles", "emit-mermaid", "check-mermaid",
    ...
    elif args.cmd == "css-book-dark":
        sys.stdout.write(css_book_dark_block(t))
