...
LANDING_CSS = """
  ...
  /* Content width — one shared max across hero / sections / one-page so the page is a CONSISTENT width
     throughout, not narrow above-and-below a wide middle. Prose stays readable via inner caps
     (.m-lead 82ch, .sec-lead 44rem); the card grids and the one-page figure use the extra width. */
  .hero-lead { max-width:min(1680px, 100%); margin:0 auto; text-align:center; }
  ...
  .hero-lead .m-lead { max-width:min(1120px, 100%); font-size:20px; line-height:1.6; margin-left:auto; margin-right:auto; }
  ...
  .onepage .sec-lead { margin-left:auto; margin-right:auto; text-align:left; max-width:44rem; }
  .onepage-fig { margin:1.4rem 0 0; }
  .onepage-fig svg { width:100%; height:auto; max-width:900px; display:block; margin:0 auto; }
  .claims-col { max-width:44rem; margin:2.4rem auto 0; text-align:left; }
  ...
  /* Desktop: the one-page section widens into a two-column info graphic — the six claims beside a sticky
     ...
     is sized by VIEWPORT HEIGHT (~80vh, capped) rather than column width: it auto-fits the screen so the
     whole diagram is visible while pinned, and its column auto-sizes to that height (the claims take the
     remaining width, floored at a comfortable measure). A viewport-width cap keeps it from crowding the
     claims on short-and-wide screens. Below this breakpoint the body is a single column in DOM order
     (intro → figure → claims), so the mobile reading sequence is preserved without duplicated markup.
     Breakpoint set by visual testing: below it the figure would be squeezed under ~490px, so the awkward
     1000–1119px range collapses to a single readable column rather than two cramped ones. */
  @media (min-width: 1120px) {
    .onepage { max-width:min(1680px, 100%); }
    .onepage-body { display:grid; grid-template-columns:minmax(22rem, 1fr) auto;
    ...
    .onepage-body .onepage-fig { grid-column:2; grid-row:1; position:sticky; top:1.6rem; margin:0;
                                 align-self:start; }
    .onepage-body .onepage-fig svg { height:min(80vh, 900px); width:auto; max-width:min(56vw, 100%);
                                     display:block; }
    .onepage-body .claim:first-child { border-top:none; padding-top:0.2rem; }
  }

  .v3-sec { max-width:min(1680px, 100%); margin:3rem auto; }
...
def _v3_onepage_figure() -> str:
    """The 'MAGE in One Page' section — heading + a short web-visitor intro at a readable measure, then a
    two-column info-graphic body at the desktop breakpoint: the six claims beside the book's canonical
    figure (book/assets/mage-method.svg, embedded as a responsive inline SVG, NOT rasterized). The figure is
    sticky within its column so the conceptual map stays beside the claims that explain it; below the
    breakpoint the body collapses to intro → figure → claims. The claims are built by `_landing_big_ideas()`
    (nested here, no longer a sibling section); their model ids still project — check_big_ideas."""
    svg = namespace_svg_ids(_inline_svg("assets/mage-method.svg"), "mage-one-page")
    claims = _landing_big_ideas()
    return (
...
def _apply_body() -> str:
    ...
    p.append(render_md(
        ...
        "[Read Chapter 4: MAGE in Motion →](book/mage-book/4.1-the-dynamics-of-mage.html) · "
    ...
    p.append(render_md(
        ...
        "advance.\n\n"
        "[Read the full method in Chapter 4 →](book/mage-book/4.1-the-dynamics-of-mage.html)"))
    p.append(render_md(
        "## Start where you are\n\n"
    ...
    p.append(render_md(
        ...
        "of delegation.\n\n"
        "## Go deeper\n\n"
        "**MAGE in Motion: Engineering Through Models — Chapter 4.** The complete treatment of the practical method: "
        "the dynamics, the worked DocAble case, brownfield recovery, and engineering the environment — "
        "validation, operations, and reusable skills. [Read Chapter 4 →](book/mage-book/4.1-the-dynamics-of-mage.html)\n\n"
...
def cmd_build(_args) -> int:
    ...
    landing_body = "\n".join([
        _v3_nav(), _v3_hero(), _v3_onepage_figure(),
