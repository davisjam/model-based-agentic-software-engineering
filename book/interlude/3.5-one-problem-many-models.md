...
<!-- figure: assets/interlude-graph-two-partitions.svg | *Two execution policies over the same remediation graph.* (a) Page-oriented execution assigns graph nodes according to where their artifacts occur, so one partition may contain computations from several dependency levels. (b) Frontier execution repeatedly dispatches the currently ready nodes: when one frontier completes, its outputs satisfy dependencies and expose the next. The remediation graph is identical; only the execution partition changes. -->
