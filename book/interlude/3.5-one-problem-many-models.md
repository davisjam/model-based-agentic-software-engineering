...
<!-- figure: assets/interlude-prediction-residual.svg | *Using prediction error as evidence.* (a) The gap between a model's prediction and the observed measurement is a residual, pointing to a wrong parameter, a wrong relation, or a missing mechanism (the numbers are illustrative). (b) The loop it drives: model, predict, implement, measure, explain the residual, refine the model. -->
...
[ref:interlude-model-family-composition] shows how they compose.

<!-- label: interlude-model-family-composition -->
<!-- figure: assets/interlude-model-family.svg | *How the model family composes.* The remediation graph and fact/IR catalogue supply dependency structure and the batch model per-call cost; together they feed the critical-path model, which yields latency predictions and a distribution choice. The media-residency model contributes an independent worker-memory bound that makes the execution design affordable, and the image-fidelity model a visual-IR cost. -->
...
The process is therefore not the linear one — design, then implement, then document with models — but a loop, in which analysis and measurement feed back into the model ([ref:interlude-modeling-cycle]).
