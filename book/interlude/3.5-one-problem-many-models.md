...
The 250 MB requirement had therefore exposed a representation problem: storage size and reasoning state had been allowed to scale together, even though most remediation operations needed only the structural portion.
...
A page tells us where content occurs, not which computations depend on which others. DocAble already represented remediation as a dependency graph. Some operations can run from the source document alone. Others consume intermediate semantic structures produced by earlier work: local interpretations may feed a document-level representation, which in turn supplies context to downstream remediation. [ref:interlude-graph-two-partitions] shows this dependency graph and the two ways it can be partitioned.
