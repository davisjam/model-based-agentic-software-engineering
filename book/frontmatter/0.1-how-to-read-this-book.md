...
- **Adopt it in an organization.** Part I → Part V (Section 5.5) → [appendix: appendix-adopting-genai]. Start with the changed engineering economics, see how several organizations are already investing in context, models, evidence, and control, then use that appendix to reason about bounded delegation and where stronger engineering structure is worth the investment. It points back into the method and theory when you need them, so this route need not pass through Parts II–IV first.
