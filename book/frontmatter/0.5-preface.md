...
This book develops MAGE through software engineering, but its underlying subject is broader: how to govern capable agents performing consequential engineering work. Agents can already inspect large software systems, use tools, make changes, run experiments, and produce implementation faster than people can inspect everything they do. The resulting problem is not merely how to make the agent more capable. It is how to engineer the environment through which that capability acts.

Nothing about that problem is unique to source code. An agent analyzing a contract, developing an engineering design, reconciling accounts, or carrying out a scientific analysis faces the same abstract difficulty when its work outruns direct human inspection: consequential knowledge may remain implicit, the agent may have to reconstruct it imperfectly, and some obligations matter too much to depend only on the producing reasoner getting them right. Purposeful representations, independent evidence and enforcement, and durable structures that preserve prior judgment are possible responses.

Software engineering is MAGE's first worked domain. It is where our engineering expertise lies, where the approach was developed, and where the evidence in this book lies. The chapters that follow therefore develop MAGE concretely through software engineering rather than claiming evidence we do not have elsewhere. Near the end of the book, we return to the more general question: which parts of the account depend on software, and which belong to the broader problem of governing agents?

Call the broader problem **agentic engineering**: the deliberate engineering of the environments through which autonomous agents perform consequential engineering work. The term names the conditions engineered around agentic work, not engineering performed by agents. Model-Based Agentic Engineering (MAGE) is the account this book develops for addressing it: make consequential knowledge explicit in purposeful representations, give selected obligations evidence and enforcement outside the producing reasoner, and preserve useful judgment in durable structures that later work can inherit.

## Why MAGE? Why Models?

<!-- gloss-only: MAGE | Model-Based Agentic Engineering — the framework of this book: engineering with a fleet of agents by binding intent to structured models the fleet reasons through, and governing what it builds with mechanisms (constraints and sensors); developed and demonstrated here through software engineering. The name packages the book's two principles, the Modeling Principle and the Alignment Principle. -->

One way to understand MAGE—Model-Based Agentic Engineering—is as a classical software-engineering move: **add a level of indirection.**[^indirection]
...
[^indirection]: Adding a level of indirection is so commonly a solution that it is sometimes jokingly called the *fundamental theorem of software engineering*.

## Where the theory came from

This book began with one hard engineering problem that emerged over months of building a real production system. Only later did I test its vocabulary against independently described systems from Cloudflare, Shopify, Spotify, Siemens, Zenseact, Uber, and GitLab. {{chapter:software-factories}} presents both views of the evidence: DocAble in depth, then the industrial reconstructions in breadth. {{chapter:theory}} asks what account explains both. [ref:research-arc] shows the evidentiary path.

<!-- label: research-arc -->
<!-- figure: assets/research-arc.svg | *From case to theory.* MAGE began with one deeply observed production system, then was compared with seven independent industrial systems. The originating case shows mechanism and sequence; the industrial cases show variation and alternative ways to realize the same moves. -->

<!-- point: resource-scarcity-pushed-mage-toward-stating-what-the-product-must-preserve | A resource-poor engineering environment made abundant human supervision unavailable, which pushed MAGE toward stating what the product must preserve and building a production system that could establish those properties without a human inspecting every realization. -->

Some of the ideas in this book emerged from a resource-poor engineering environment. I built DocAble by myself — just me and Claude, who produces software far faster than I can inspect. The resulting system still had to satisfy consequential accessibility requirements. I could not solve that problem by supervising the agents more closely; human attention was precisely the resource I did not have. As I watched my weekly Apple Watch sleep report trendline decay, I instead had to make the requirements and other consequential properties explicit enough that the production system could preserve and evaluate them without requiring me to inspect every realization. Much of MAGE grew from following that constraint seriously. In retrospect, the lack of resources was useful: it made solutions that depended on abundant human supervision unavailable. As we will see in {{chapter:software-factories}}, commercial applications of agentic technology have often developed differently. Large engineering organizations can use their existing human staff as part of the production system, placing people at review points, escalation boundaries, and other consequential decisions while engineering the processes around them. I could not. That difference in resources helped push MAGE toward a question that their production systems could more readily leave to human judgment: what properties must the product preserve, and how can the production system establish them without requiring a human to inspect every realization?
...
This creates a supervision problem. Increasing the capacity to produce software does not automatically increase the capacity to understand, evaluate, or govern its production. When supervision remains attached primarily to the implementation being produced, human attention must scale with the volume of implementation that needs to be understood and evaluated. But does the quality team truly need to examine every widget? {{chapter:software-factories}} returns to this problem at the scale of the production system.
...
The originating system is DocAble, a production service for remediating inaccessible documents. By the period studied here, coding agents had written roughly {{prod_loc}} lines of production code. The environment governing that code—tests, checks, agent infrastructure, and documentation—had grown about three times larger, to {{support_loc}} lines.

Although DocAble is a tidy little demonstration of techniques from natural language processing and compilers, the most interesting thing about DocAble is not DocAble. It is MAGE. MAGE predicts that one engineer can use agents to build and sustain a production system whose implementation would ordinarily require a software team. DocAble shows this in action. It also poses the question this book answers: once agents write almost all of the code, what engineering remains?

DocAble therefore serves as the book's deepest worked example. Chapters {{chapter:modeling|num}} and {{chapter:alignment|num}} examine the mature system from the perspectives of Modeling and Alignment. {{chapter:one-problem-many-models}} puts those mechanisms in motion, tracing engineering problems through successive models, enforcement, work, and governance conversion. {{chapter:software-factories}} then takes the production-system view: it reconstructs DocAble's software factory and compares it with independently documented industrial factories.

## Evidence and scope

MAGE draws on two kinds of evidence. The first is a single longitudinal case: one production system, built by one engineer using one vendor's agent ecosystem over several months and used in production by me and my colleagues at Purdue. Numbers from that case are observations, not measured laws. The second is a set of reconstructions based on independent industrial accounts. Those accounts show less of how each system evolved, but more variation in technical and organizational choices, including systems that rely more heavily on explicit models and systems that rely more heavily on controls and human judgment. {{chapter:software-factories}} presents both sources, states their limits, and ties empirical claims to supporting data or citations.
