...
_THESIS_LEAD_RE = re.compile(r"^\*\*The\b.*?\b(?:Thesis|Principle)\.\*\*", re.S)
...
def _render_blockquote(raw: str, is_def: bool = False, is_pullquote: bool = False,
    ...
    `<!-- principlebox -->` marker (`is_principlebox`) becomes a part-opener GREEN box with a full 4-side frame and,
...
def render_chapter(chapter: ir.Chapter, ctx: _EmitCtx) -> str:
    ...
    for i, b in enumerate(blocks):
        ...
        if b.kind is ir.BlockKind.DIRECTIVE:
            # A Part-intro's `<!-- principlebox -->` renders on the orientation VERSO (the divider builder reads
            # it via `_extract_principlebox_raw`, §G-4), so skip the marker AND its blockquote here — the box
...
//
// Side-float wrap ISOLATION POINT — the vendored wrap-it engine (`_wr-*`, `_bw-aside-width`,
// `_WR-MIN-STRIP-W`, `_wr-wrap-right`) now lives in the tracked shared module
// /book/typst/side-float.typ, imported here so the Software Engineering Handbook's wrapped
// figure/table component (handbook/typst/components.typ) reuses the SAME engine instead of a
// second copy. Provenance, the WHY-vendored rationale, and the measurement-neutralisation notes
// moved with the code — read them there. `weight-aside` below and the handbook component are the
// only callers; both compile with `--root` at the catalogue root, so the import path is stable.
#import "/book/typst/side-float.typ": *
...
#: The in-part UNNUMBERED codas — the terminal closings (the per-Part Summary
...
def _extract_principlebox_raw(ch: "ir.Chapter") -> "str | None":
    """The raw markdown of a Part-intro's `<!-- principlebox -->` blockquote, or None when the intro has none.
    ...
    box prints exactly once. The `<!-- principlebox -->` marker and its blockquote stay file-resident in
...
def emit_document(slugs: list[str], root: pathlib.Path | None = None, *, with_frontmatter: bool = False,
    ...
    if with_frontmatter:
        ...
        parts.append(
            ...
            "            #box(width: 1fr, inset: (x: 0.4em), align(bottom, repeat(gap: 3pt, justify: false)[#text(fill: dt.muted)[.]]))\n"
            "            #text(font: dt.font-body, weight: \"regular\", size: sz, fill: dt.ink)[#pg]\n"
            "          ]\n"
            "        ]\n"
            "      ]\n"
            "    }\n"
