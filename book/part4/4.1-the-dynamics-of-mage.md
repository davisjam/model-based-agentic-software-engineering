...
Cheap implementation changes engineering experience in two related ways.

<!-- box-family: canonical -->
> ### Engineering phenomena — Realization and connection amplification {#phenomena-amplification}
>
...
>
> Compressed exploration produces compressed experience. The first amplification generates the evidence; the second is what happens when that evidence arrives close enough together for its relationships to show. Neither is something an engineer performs. Both describe conditions the engineer now works inside.
...
The engineer still decides which alternatives are worth exploring and which apparent connections matter. When a connection reveals a recurring engineering lesson, governance conversion provides the next move: preserve that lesson in the environment so that later work inherits it. Once that knowledge is explicit, it can compound: later work begins with the model, constraint, evidence, or mechanism produced by earlier experience rather than reconstructing the same judgment.
