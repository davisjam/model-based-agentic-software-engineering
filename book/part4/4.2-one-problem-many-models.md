...
The interest here is how those techniques enter a MAGE process. The engineer does not need to know the final model or architecture in advance. A problem may begin as an observation: a file fails, a service slows down, or a new requirement exceeds the system's current range. Modeling makes one consequential relation explicit; analysis suggests what to change or measure; active alignment changes the realization; and the resulting implementation supplies new evidence. What becomes understood can then remain in the GEE as a constraint on subsequent change.

The DocAble case follows that loop from a memory failure through several changes in representation and architecture. It shows the dynamic process described in {{sec:the-dynamics-of-mage}} in brownfield form: discover a consequential property, model it, change the system under the constraints already understood, and retain the resulting knowledge as the governed surface expands.
...
In the terminology introduced in {{sec:the-dynamics-of-mage}}, this is active alignment. The residency model does not merely describe the desired architecture; it states the property toward which the realization is being changed. Once the new architecture satisfies that property, its invariants remain in the GEE as passive alignment. Later changes should not be allowed to reintroduce dependence between total media size and persistent worker memory.
...
This is why model-based engineering need not mean design, then implement, then document with models. Implementation, measurement, and analysis can expose the next thing worth representing ([ref:interlude-modeling-cycle]).
...
The DocAble sequence also illustrates the dynamics introduced at the beginning of this chapter. The memory model became consequential and drove active alignment of the implementation. Once established, its invariants persisted as constraints on subsequent work. That protected property then made it safer to reconsider execution structure. New models expanded the governed surface without requiring the engineer to have predicted the final model family in advance.

This is also why MAGE does not require complete modeling before implementation. In a brownfield system such as DocAble, existing implementation and observed failures provide evidence from which models can be induced. In a greenfield system, cheap prototypes can play the same epistemic role. Implementation can move quickly, expose a question, and supply measurements while the engineer builds just enough representation to reason about what matters next.
...
None of the engineering techniques in this chapter requires MAGE. An experienced engineer might have recognized some of them earlier, chosen different representations, or found a better architecture. MAGE offers something different: a disciplined way for the engineer and agent to discover what the particular system needs while preserving what has already become consequential. Observations become questions; questions motivate models; models support predictions and constraints; active alignment changes the realization; passive alignment preserves what has been established; and discrepancies expose what remains poorly understood.

The resulting GEE need not appear all at once. It can accrete. A small alignment kernel protects what cannot safely be lost. Engineering proceeds within that protected region. New knowledge becomes new models; active alignment brings the realization into correspondence with them; and the resulting constraints join the environment that governs the next change.
