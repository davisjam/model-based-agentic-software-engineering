...
| **Model-sync efficacy** | formative | Whether the drift and parity gates keep model equal to code. | Watch that map-equals-territory holds. | Gates stay green. | [2.8](2.8-keeping-models-in-sync.html) |
...
| **MBSE navigation token-savings** | summative | Tokens spent to reach an answer, model on versus off. | Certify the model earned its context budget. | Lower with the model on. | [2.1](2.1-the-executable-zoo.html) |
