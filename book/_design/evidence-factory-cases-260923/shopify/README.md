...
**[C33]** Shopify's human-authority doctrine is stated as a responsibility claim, not an evidence-adequacy claim: "Machines can inform a decision but cannot take responsibility for it" [E50] `[third-party paraphrase]`. Read with [E24]'s ownership rule, the record grounds the human gate in accountability rather than in any stated deficiency of the machinery — a different reason than Uber's stated-interim posture ([UBER E30]), and one that predicts the gate persists as capability improves.

**[C34]** Shopify's security lane defines completion against authoritative state rather than producer report: "done" holds only when "the source system records the outcome, the default branch reflects it, and the tracker and ledger agree" [E32], and River "treats every ledger entry as a claim to be checked, not a fact" [E35]. With the freshness-gated queue [E23], this is the corpus's most explicit statement that admission evidence must be regenerated from live state rather than accepted from the agent that produced the work — and it was adopted after the one documented run in which the prompt-stated obligation failed and the failure surfaced only through River's own report [E27]. `[E23] [E27] [E32] [E35]`
...
2. **Whether the nightly "dreaming" loop exists as described.** [E48] is two third-party digests of an inaccessible podcast transcript. Neither Shopify engineering post mentions self-rewriting instruction files. Unresolved. Marker: **unclear** — two independent third-party digests describe it and no first-party source mentions it; the sources conflict in weight, not in content.
...
15. Whether "draft-only behavior" and "human merge authority" are enforced mechanically (e.g., River lacking VCS merge permission) or expressed only in prompts is **unclear**: [E25] locates both in "River prompts and skills," [E26] mandates that closure-determining rules "must be enforced in code" and lists candidates that "should become deterministic as the workflow matures," and no source states which of the admission controls have actually been moved from prompt to code.
16. **Whether cross-model verification extends beyond the AppSec harness.** [E37] documents the Verifier stage using a different model than the Hunter, with a stated decorrelation motive ("adversarial review… prevents blind spots"), for the Dispatch pipeline. Whether any of River's general workflows use a different model for verification than for generation is not publicly stated.
17. **Post-admission containment is not described.** [E32]'s post-merge check — the default branch reflects the outcome and tracker and ledger agree — is *detection* of an unfinished or regressed fix, not containment of a wrong one. Nothing describes staged rollout, feature gating, or rollback for a River-coauthored change that later proves wrong, in the security lane or generally. Not publicly stated. (Blast-radius machinery that *is* documented is fabrication-phase only: the harness outside the sandbox [E15], deterministic ownership of credentials/Git/storage [E43], draft-only output [E25].)
18. **No control retirement.** The record documents controls being added and hardened — guarantees migrated from prompt to code after a violation [E26][E27], a harness framed as outlasting the model [S5] — and no instance of a control weakened or retired as model capability improved. Not publicly stated.
19. **The drift-fixing automation is not publicly stated.** Lütke says instruction-file drift is "fix[ed] with automation" [E45]; no source describes that automation — what it checks, when it runs, or whether it repairs drift or merely detects it. The maintenance regime for the corpus on which the factory's learning claim rests ([E5], [E7], [E20]) is otherwise social: people watching sessions and writing files.
