...
None of this is a new representational tradition. Requirements engineering, architecture, programming languages, formal methods, model-driven software engineering, model-based systems engineering, knowledge representation, and AI have long used explicit representations to make selected questions tractable.[note: External-representation moves in recent AI: LLM+P externalizes planning into a classical planner [cite: liu2023llmp]; MemGPT externalizes state into managed memory tiers [cite: packer2023memgpt]; Graph of Thoughts externalizes intermediate reasoning as a graph [cite: besta2024got]; and WorldCoder externalizes a persistent world model as code [cite: tang2024worldcoder].] A type can exclude an invalid local state [cite: pierce2002]; an intermediate representation can expose the structure a transformation needs [cite: appel-compilers]; a systems model can connect requirements, behavior, architecture, and evidence [cite: friedenthal-sysml] [cite: brambilla2017]. What changes with agentic software engineering is the economic importance of these representations: they can become active surfaces through which autonomous work is informed, scoped, checked, and reviewed.
...
A software factory should therefore be evaluated not only by how much software it can produce, but by whether the organization can continue to understand, assure, and take responsibility for the systems it produces.
