...
MAGE proposes a directional account of agentic software engineering. Its central claim is that agentic capacity acts through the engineered environment surrounding the work. Increasing implementation capacity should therefore have different effects depending on the representations, evidence, enforcement, and other engineering structure through which it acts.

Three claims carry most of the theory.
...
These claims are conditional. MAGE does not predict that more modeling is always better, that every obligation should become deterministic, or that accumulated engineering machinery is intrinsically valuable. Representation, enforcement, and engineering capital cost something to build, maintain, coordinate, and use. Some consequential judgments remain contextual or too hard to observe reliably and properly remain matters for expert judgment.

The research agenda in this chapter identifies ways to test these claims and the mechanisms proposed to explain them. MAGE is not presented as a finished law of agentic software engineering. It is an explanation that can be tested, refined, bounded, or rejected.

{{chapter:profession}} asks what follows for software engineering—and for the engineer—if that explanation is substantially right.
